...
try:
    import munge_sumstats as legacy_munge
except ImportError:  # pragma: no cover - package import fallback
    from .. import munge_sumstats as legacy_munge  # type: ignore[no-redef]

from .config import CommonConfig, MungeConfig


parser = legacy_munge.parser
null_values = legacy_munge.null_values
default_cnames = legacy_munge.default_cnames
describe_cname = legacy_munge.describe_cname
numeric_cols = legacy_munge.numeric_cols
read_header = legacy_munge.read_header
get_cname_map = legacy_munge.get_cname_map
get_compression = legacy_munge.get_compression
clean_header = legacy_munge.clean_header
filter_pvals = legacy_munge.filter_pvals
filter_info = legacy_munge.filter_info
filter_frq = legacy_munge.filter_frq
filter_alleles = legacy_munge.filter_alleles
parse_dat = legacy_munge.parse_dat
process_n = legacy_munge.process_n
p_to_z = legacy_munge.p_to_z
check_median = legacy_munge.check_median
parse_flag_cnames = legacy_munge.parse_flag_cnames
allele_merge = legacy_munge.allele_merge
munge_sumstats = legacy_munge.munge_sumstats
...
class SumstatsMunger:
    ...
    def run(
        ...
        data = legacy_munge.munge_sumstats(args, p=True)
        table = SumstatsTable(
...
def main(argv: list[str] | None = None):
    ...
    return legacy_munge.munge_sumstats(args, p=True)


def _count_data_rows(path: str) -> int:
    openfunc, _compression = legacy_munge.get_compression(path)
