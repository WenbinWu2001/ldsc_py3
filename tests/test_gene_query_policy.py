...
def test_coverage_uses_unique_selected_genes_and_leaves_support_unknown(tmp_path, policy):
    ...
    for name, content in [("full", "G1\nONE\n"), ("partial", "G1\nG2\n"),
        ...
        sources.append(path)
    batch = resolve_gene_fixture(sources, GeneCatalog.load(catalog), resolution_policy=policy, output_dir=tmp_path)
    ...
    assert fixture_gene_audit(batch).loc[fixture_gene_audit(batch).canonical_gene_id.eq("G2"), "coverage_status"].tolist() == ["uncovered", "uncovered"]
    assert not fixture_gene_audit(batch).disposition.eq("unsupported").any()
...
def test_missing_support_evidence_is_unknown_not_zero(tmp_path):
    ...
    batch = resolve_gene_fixture([source], GeneCatalog.load(catalog), output_dir=tmp_path).with_snp_support(pd.Series(dtype="Int64"))
    assert fixture_gene_audit(batch).disposition.tolist() == ["retained"]
...
def test_exclusions_precede_coverage_and_all_excluded_is_empty(tmp_path):
    ...
    full.write_text("MHC\nG1\nONE\n")
    batch = resolve_gene_fixture([excluded, full], GeneCatalog.load(catalog), gene_exclude_regions="mhc", output_dir=tmp_path)
    batch, errors = assess_gene_coverage(batch, ["22"])
    assert not errors
    ...
    assert fixture_gene_audit(batch).loc[fixture_gene_audit(batch).input_gene.eq("MHC"), "coverage_status"].isna().all()
