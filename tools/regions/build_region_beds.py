...
# The `mhc` preset: broad GWAS exclusion window, build-consistent (chr6:25-35 Mb
# in both builds; hg19 25-35 Mb lifts to hg38 ~25.00-35.03 Mb, so the same numeric
# window covers the same biology). Pinned constant.
MHC = {
    "hg19": [("6", 25_000_000, 35_000_000)],
    "hg38": [("6", 25_000_000, 35_000_000)],
}
_MHC_PROVENANCE = {
    "hg19": "MHC broad GWAS exclusion window, hg19 chr6:25-35Mb, pinned constant; 0-based half-open",
    "hg38": "MHC broad GWAS exclusion window, hg38 chr6:25-35Mb, pinned constant (hg19 25-35Mb lifts here); 0-based half-open",
}
...
def main() -> None:
    ...
    for build in ("hg19", "hg38"):
        write_bed(OUT_DIR / f"mhc.{build}.bed", MHC[build], _MHC_PROVENANCE[build])
