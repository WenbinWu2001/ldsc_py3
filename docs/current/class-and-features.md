...
| Build a gene LD-score index | `ldsc build-gene-ldscore-index` | `GeneLDScoreIndexBuildConfig`, `build_gene_ldscore_index()`, `load_gene_ldscore_index()` | baseline annotations inner-joined to PLINK by rsID, optional custom regression SNPs, immutable gene/index settings | one complete validated index directory with `index_id`, catalog, chromosome shards, successful-build JSON and completed log; live/failed logs and locking use hidden sibling build state |
