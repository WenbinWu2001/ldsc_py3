...
| Build parquet reference panels | `ldsc build-ref-panel` | `ReferencePanelBuilder`, `run_build_ref_panel()` | PLINK prefix, optional liftover chains, conditional genetic maps, optional keep/restrict files; optional restriction identifier/build overrides | per-build `r2/chr*_r2.parquet` and `meta/chr*_meta.tsv.gz` artifacts |
...
| `MungeConfig` | raw-sumstats input, optional `sumstats_snps_file` row restriction, column hints, munging thresholds, and output settings |
