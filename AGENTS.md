...
- Keep one CLI surface: `ldsc` with `annotate`, `ldscore`, `build-ref-panel`, `build-gene-ldscore-index`, `convert-ldsc2-ldscores`, `munge-sumstats`, `h2`, `partitioned-h2`, `quantile-h2`, `rg`, and `query-r2`. `ldsc.cli` dispatches; it must not gain numerical logic.
...
- Every CLI subcommand requires `--output-dir`. Commands write their canonical artifact families there; `munge-sumstats --infer-only` is the sole no-write mode but still requires the destination argument for a uniform CLI contract.
