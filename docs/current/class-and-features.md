...
| Build a gene LD-score index | `ldsc build-gene-ldscore-index` | `GeneLDScoreIndexBuildConfig`, `build_gene_ldscore_index()`, `load_gene_ldscore_index()` | baseline annotations inner-joined to PLINK by rsID, optional custom regression SNPs, immutable gene/index settings | workers durably stage distinct chromosome payloads and release their records; the coordinator publishes one complete validated index with `index_id`, catalog, canonical chromosome order, successful-build JSON, and completed log; private stages are never resumable, and live/failed logs plus locking use hidden sibling build state |
