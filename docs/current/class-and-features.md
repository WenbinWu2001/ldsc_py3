...
| Build a gene LD-score index | `ldsc build-gene-ldscore-index` | `GeneLDScoreIndexBuildConfig`, `build_gene_ldscore_index()`, `load_gene_ldscore_index()` | strict baseline annotations plus a matching PLINK suite and immutable gene profile settings | shared `common/` data plus one validated `profiles/<profile>/` disjoint-atom operator |
