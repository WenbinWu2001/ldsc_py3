...
`RegressionRunner.estimate_partitioned_h2_batch(..., output_dir=..., query_batch_size=1000)` shares trait/baseline preparation, fits each query separately, and stages completed details immediately. After stable summary sorting, the writer publishes the canonical per-query tree. The returned `PartitionedH2BatchResult` contains `summary`, `output_paths`, and persistent `per_query_artifacts` paths. The old annotation-bundle argument, retained-detail dictionaries, and `include_full_partitioned_h2` option are removed. Low-level writer callers pass `PartitionedH2FitArtifacts` through `per_query_artifacts`; `PartitionedH2OutputConfig.write_per_query_results` remains its publication control. Individual estimator methods retain their text `summary()` interface.
