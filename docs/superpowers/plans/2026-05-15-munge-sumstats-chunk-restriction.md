...
2. Add a pre-parse preparation function:
   ...
   - load `set[str]` once in base `rsid` mode;
   - load packed `set[int]` once in base `chr_pos` mode using the streaming
     restriction reader;
   - use the shared restriction identity reader for allele-aware modes and for
     allele-bearing restriction files;
   - raise before raw parsing if the keep-set is empty.
3. Add early source-build resolution for every `chr_pos`-family run with
...
4. Move coordinate normalization/drop behavior into the chunk path for
   `chr_pos`-family modes. Use the shared coordinate policy to drop missing or
   invalid coordinates with warnings. If early auto inference found 0-based
   coordinates, shift chunk POS to canonical 1-based coordinates before
   keep-list matching.
...
8. Keep post-concat steps for shared identity cleanup, `process_n()`, P-to-Z,
...
- Base `chr_pos` filtering uses packed keys and does not call `CHR:POS`
  string-key helpers.
- Non-kept rows do not reach post-concat stages such as shared identity cleanup.
- `chr_pos`-family `genome_build=auto` resolves before parsing and uses inferred
...
- Missing/invalid CHR/POS rows in `chr_pos`-family modes are dropped with separate
  coordinate warnings.
