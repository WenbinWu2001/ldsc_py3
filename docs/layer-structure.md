...
| Shared normalization and inference | `path_resolution.py` owns path-token language: exact paths, globs, and explicit `@` chromosome suites. `column_inference.py` owns header aliases, SNP identifier modes, genome-build aliases, and strict internal artifact schemas. `chromosome_inference.py` owns chromosome normalization and sort order. `genome_build_inference.py` infers `hg19`/`hg38` for `chr_pos` inputs when auto mode is used. | Annotation code uses shared path and column inference to resolve `.annot(.gz)` inputs, BED inputs, required metadata columns, restriction files, and chromosome shards. | Builder and runtime loaders use shared path resolution for PLINK prefixes, optional map and chain inputs, parquet R2 groups, metadata sidecars, keep files, and SNP restrictions. The build-ref-panel CLI/convenience wrapper requires an explicit SNP identifier only for SNP restriction files. | The LD-score workflow uses path resolution for annotation groups, PLINK prefixes, parquet R2 groups, metadata sidecars, keep files, and SNP restrictions; it uses column and genome-build inference before calling kernels. | Munging resolves one raw file and one optional merge-alleles file; raw headers are inferred through `column_inference.py`, while curated `.sumstats` artifacts are loaded with stricter internal column specs and unknown config provenance. | Regression treats `ldscore_dir` as a literal result directory and sumstats inputs as scalar paths; it validates config compatibility before merging data when both snapshots are known. | Output code uses `ensure_output_directory()` and `ensure_output_parent_directory()` from `path_resolution.py`; output paths are literal destinations, not input globs. |
