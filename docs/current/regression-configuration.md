...
| `samp_prev`, `pop_prev` | **Reserved** — intended for observed→liability-scale conversion of binary-trait h². **Not currently wired to the CLI and not consumed by the regression module.** | Not usable yet; liability-scale conversion is not implemented in this module. *(Placeholder: document the prevalence-input workflow once implemented.)* |
