...
- LD-score output also writes `ldscore.overlap.parquet`, the long-form annotation
  overlap matrix consumed by `partitioned-h2`. `metadata.json` records its path in
