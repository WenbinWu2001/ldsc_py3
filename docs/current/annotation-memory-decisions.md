...
6. Prepare and release query annotations within the batch lifecycle to reduce peak scratch where the input route permits it. Retain the existing dense annotation representation. Lower peak scratch does not imply fewer total annotation bytes written across all batches. There is no required follow-up session to implement sparse storage or another annotation representation.
