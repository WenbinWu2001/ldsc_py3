...
| Common-count threshold | `LDScoreConfig.common_maf_min` | `--common-maf-min` | During count-vector and overlap-matrix computation after LD scores are computed | Defines the common-SNP universe with strict `MAF > common_maf_min` (matching legacy `0.05 < FRQ < 0.95`); affects `common_reference_snp_count(s)` and the common-universe overlap matrix, but not LD rows, LD scores, or the stored regression-universe LD score |
