...
| `--format` | input metadata | no | raw summary-statistics format profile | One of `auto`, `plain`, `daner-old`, `daner-new`, or `vcf`; defaults to `auto`, which detects common plain text, old DANER, new DANER, and VCF-style headers. Conflicts with incompatible legacy DANER flags are rejected. |
