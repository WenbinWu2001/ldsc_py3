...
- group inputs such as `--baseline-annot-sources`, `--query-annot-sources`, `--query-annot-bed-sources`, `--r2-sources`, and `--metadata-sources` may resolve to many files; package-built parquet panels can be supplied as one build directory with `--ref-panel-dir`
