...
| `--control-gene-list-source` | input | no | fixed gene control | Adds the fixed baseline `gene_control` for live or indexed gene-list runs; defaults to `all-protein-coding`. Use `none` to disable it or supply one custom control-list path. Non-default choices require a gene-list run. |
